...
if ( ! class_exists( 'loadmoreStore' ) ) {
	class loadmoreStore {
        ...
        public function __construct() {
            ...
            // Search Ajax
            add_action('wp_ajax_loadmore_ajax_action', array( $this, 'load_more_ajax_callback') ); // For logged in users
            add_action('wp_ajax_nopriv_loadmore_ajax_action', array( $this, 'load_more_ajax_callback') ); // For non-logged in users
        }
         ...
         * @return void
         */
        function load_more_ajax_callback() {
            // Your AJAX logic here
            // here we are verifying does this request is post back and have correct nonce
            if ( isset( $_REQUEST ) && wp_verify_nonce( $_REQUEST['security'], 'loadmore_nonce_key' ) ) {

            // Return the response (example: "AJAX request successful")
            //echo "AJAX request successful";

            // String to array
            $page = $_REQUEST['values'];
			...
			if ( $the_query->have_posts() ) {
				...
				while ( $the_query->have_posts() ) :
					...
					$output .= '<div class="splide_abc razziFadeInUp" style="animation-delay: '.$delay.'ms;">
